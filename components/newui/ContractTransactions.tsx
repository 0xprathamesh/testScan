...
const ContractTransactions: React.FC<TransactionProps> = ({ address }) => {
        ...
        const transactionData = response.items.map((item: any) => ({
          ...
          to:
            item.to?.hash && item.to?.name
              ...
              ? parseAddress(item.to?.hash)
              : "Unknown",
          value: (parseInt(item.value) / 10 ** 18).toFixed(1),
          block: item.block,
          timestamp: item.timestamp,
          status: item.status,
        }));
  ...
  return (
    <div className="bg-white rounded-3xl p-4 font-inter">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold">Activity</h2>
      </div>
      <table className="w-full table-auto border-collapse">
        {/* <thead>
          <tr className="bg-gray-100 text-left">
            <th className="p-4">Method & Hash</th>
            ...
            <tr key={index} className="border-b">
              ...
              <td className="">
                <div className="flex items-center">
                  ...
                  <div className="ml-2">
                    <p className="font-normal tracking-wider">{tx.method}</p>
                    <p className="text-sm font-semibold text-[#06afe8] flex items-center">
              ...
              {/* From */}
              <td className="p-4">
                <div className="text-sm font-light text-blue flex items-center">
                  <Link href={`/newui/address/${tx.from}`}>
                    {parseAddress(tx.from)}
                  </Link>
                  <FiCopy
                    className="ml-2 cursor-pointer text-[#8a98ad]"
                    onClick={() => navigator.clipboard.writeText(tx.from)}
                  />
                </div>
              </td>
              ...
              {/* To */}
              <td className="p-4">
                <div className="text-sm  text-blue font-light flex items-center">
                  {tx.to}
                  <FiCopy
                    className="ml-2 cursor-pointer text-[#8a98ad]"
                    onClick={() => navigator.clipboard.writeText(tx.to)}
              ...
              {/* Value */}
              <td className="p-4 text-sm">{tx.value} XDC</td>
